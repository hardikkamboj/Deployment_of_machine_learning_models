...
class Pipeline:
    ...
    def find_imputation_replacement(self):
        ...
        for var in self.numerical_to_impute:
            replacement = self.X_train[var].mode()[0]
            self.imputing_dict[var] = replacement
    ...
    def find_frequent_categories(self):
        ...
        for var in self.categorical_encode:
            tmp = pd.DataFrame(self.X_train[var].value_counts() / len(self.X_train))
            self.frequent_category_dict[var] =  list(tmp[tmp[var] > self.percentage].index)
    ...
    def find_categorical_mappings(self):
        ...
        for var in self.categorical_encode:
            ordered_labels = self.X_train.groupby(var)[self.target].mean().sort_values().index
            ordinal_labels = {k:i for i,k in enumerate(ordered_labels,0)}
            self.encoding_dict[var] = ordinal_labels
        return self    
    ...
    def capture_elapsed_years(self, df):
        ...
        df[self.year_variable] =   df[self.ref_variable] - df[self.year_variable]
    ...
    def fit(self, data):
        ...
        #numerical
        for var in self.numerical_to_impute:
            self.X_train[var] = self.X_train[var].fillna(self.imputing_dict[var])
            self.X_test[var] = self.X_test[var].fillna(self.imputing_dict[var])

    
        #capture elapsed time
        self.X_train = self.capture_elapsed_years(self.X_train)
        self.X_test = self.capture_elapsed_years(self.X_test)
        ...
        #find categorical encoding
        self.find_categorical_mappings()

        #map the categorical mapping
    ...
    def transform(self, data):
        ...
        #impute num
        for var in self.numerical_to_impute:
            data[var] = data[var].fillna(self.imputing_dict[var])

        #capture elapsed time
        data = self.capture_elapsed_years(data)
